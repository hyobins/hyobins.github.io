...
아래 커맨드는 influxdb의 초기 설정(user, pw, org, bucket) 을 완료해 주었다. <br>
여기서 하지 않더라도 docker container에 접속 후 ```influx setup``` 명렁어 또는, <IP번호>:<8086>에 접속하여 설정할 수 있다.
...
1. Telegraf plugin  
2. InfluxDB v2 API (http request) 
3. ```influx``` CLI 
4. InfluxDB Web UI 
5. InfluxDB v2 API client Library(Go, Python, C++ 등)
